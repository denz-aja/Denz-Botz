...
END:VCARD`.trim()
    ...
    */
     ...
     */
    send5ButVid: { 
             async value (jid , text = '' , footer = '', vid, display1, url, call, number, display2, id1, display3, id2, display4, id3, buffer, options = {}) {
    ...
    let lol = await conn.resize(file, 300, 150)
    let btn = [{
                                urlButton: {
                                ...
                                quickReplyButton: {
                                    displayText: display4, 
                                    id: id3
                                }
                            }]
    conn.sendMessage(jid, { video: vid, jpegThumbnail: lol, caption: text, footer: footer, templateButtons: btn, ...options })
   } }, 
    /** Send Button 5 Location
       ...
       */
      send5ButLoc: { 
      async value (jid , text = '' , footer = '', buffer, display1, url, call, number, display2, id1, display3, id2, display4, id3, options = {}) {
      	let type = await conn.getFile(buffer)
            let { res, data: file } = type
            if (res && res.status !== 200 || file.length <= 65536) {
            try { throw { json: JSON.parse(file.toString()) } }
      ...
      let tyt = await conn.resize(file, 300, 150)
      let btn = [{
                                urlButton: {
                                    displayText: display1,
                                    url: url
      ...
      conn.sendMessage(jid, { location: { jpegThumbnail: tyt }, caption: text, footer: footer, templateButtons: btn, ...options })
      }}, 
      /** Send Button 5 Image
     *
     * @param {*} jid
     ...
     */
    send5ButImg: {
     async value (jid , text = '' , footer = '', img, display1, url, call, number, display2, id1, display3, id2, display4, id3, options = {}) {
     	let type = await conn.getFile(img)
            let { res, data: file } = type
            if (res && res.status !== 200 || file.length <= 65536) {
            try { throw { json: JSON.parse(file.toString()) } }
            catch (e) { if (e.json) throw e.json }
    ...
    conn.sendMessage(jid, { image: file, caption: text, footer: footer, templateButtons: btn, ...options })
    }
}, 
        /**
     * send Button Img
...
sendButtonVid: {
    ...
    sendBut: {
        async value (jid, content, footerText, button1, id1, quoted, options) {
                ...
                conn.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: 86400, contextInfo: { mentionedJid: conn.parseMention(content + footerText), forwardingScore: 555, isForwarded: true }, ...options })
            }
        },
        send2Button: {
        ...
        async value (jid, content, footerText, button1, id1, button2, id2, button3, id3, quoted, options) {
                ...
                conn.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: 86400, contextInfo: { mentions: conn.parseMention(content + footerText), forwardingScore: 555, isForwarded: true }, ...options })
            }
        },
        /**
    *status 
    ...
    async value(status) {
        ...
        sendButtonDoc: {   
            ...
            async value (jid, content, footerText, filename, button1, id1, quoted, options) {
              ...
              conn.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: global.ephemeral, forwardingScore: 555, isForwarded: true, ...options })
            }
            },
            send2ButtonDoc: {   
                /**
    ...
    */
    send3TemplateButtonImg: {
        async value (jid, buffer, content, footerText, button1, id1, button2, id2, button3, id3, quoted, options) {
                ...
                conn.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: 86400, contextInfo: { mentionedJid: conn.parseMention(content + footerText), forwardingScore: 555, isForwarded: true }, ...options, ephemeralExpiration: 86400 } )
            }
        },
        sendTemplateButtonFakeImg: {
            async value (jid, buffer, content, footerText, btn1, id1, options) {
                    ...
                    conn.sendMessage(jid, key, { ephemeralExpiration: 86400, mentions: conn.parseMention(content + footerText), contextInfo: { forwardingScore: 555, isForwarded: true }, ...options })
                }
            ...
            async value (jid, buffer, content, footerText, btn1, id1, btn2, id2, quoted, options) {
                    ...
                    conn.sendMessage(jid, key, { quoted, ephemeralExpiration: 86400, contextInfo: { mentions: conn.parseMention(content + footerText), forwardingScore: 555, isForwarded: true } })
                }
            },
        send3TemplateButtonFakeImg: {
            ...
            conn.sendMessage(jid, key, { quoted, ephemeralExpiration: 86400, contextInfo: { forwardingScore: 555, isForwarded: true, mentions: conn.parseMention(content) } })
        }
    },
    ...
    */
                    async value (jid, buffer, content, footerText, button1, id1, quoted, options) {
                            ...
                            conn.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: global.ephemeral, contextInfo: { mentionedJid: conn.parseMention(content + footerText), forwardingScore: 555, isForwarded: true }, ...options, ephemeralExpiration: global.ephemeral} )
                        }
                    },
           ...
           send3TemplateButtonLoc: {
                        ...
                        send2ButtonImgDoc: { 
                            async value (jid, buffer, contentText, footerText, button1, id1, button2, id2, quoted, options) {
                                    ...
                                    const buttonMessage = {
                                        image: file,
                                        document: { url: "https://github.com/denz-aja"},
                                        mimetype: global.doc,
                                        fileName: global.ucapan,
                                        fileLength: 999999999999999,
                                        pageCount: 999,
