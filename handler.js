        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (m.chat in global.db.data.chats || m.sender in global.db.data.users) {
                    let chat = global.db.data.chats[m.chat]
                    let user = global.db.data.users[m.sender]
                    if (name != '_unbanchat.js' && name != '_exec.js' && name != '_exec2.js' && name != '_delete.js' && chat?.isBanned)
                        return // Except this
                    if (name != '_unbanuser.js' && user?.banned)
                        return
                }
            ...
            if (chat.welcome) {
              ...
              let lea = await new knights.Goodbye()
                ...
                .toAttachment()
                       ...
                       await this.sendHydrated(id, global.ucapan, text, action === 'add' ? wel.toBuffer() : lea.toBuffer(), sgc, (action == 'add' ? 'Welcome' : 'Bye'), user.split`@`[0], 'Nomor', [
      [action == 'add' ? 'Welcome' : 'Sayonaraaa', action === 'add' ? '.intro' : 'bilek']], null, fkontak, { mentions: [user] })
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (groupUpdate.desc) text = (chats.sDesc || this.sDesc || conn.sDesc || 'Deskripsi group telah diubah!!\n@desc').replace('@desc', groupUpdate.desc)
        if (groupUpdate.subject) text = (chats.sSubject || this.sSubject || conn.sSubject || 'Judul group telah diubah!!\n@subject').replace('@subject', groupUpdate.subject)
        if (groupUpdate.icon) text = (chats.sIcon || this.sIcon || conn.sIcon || 'Profil group telah diubah!!').replace('@icon', groupUpdate.icon)
        if (groupUpdate.revoke) text = (chats.sRevoke || this.sRevoke || conn.sRevoke || 'Link group telah diubah!!\n@revoke').replace('@revoke', groupUpdate.revoke)
        if (groupUpdate.announce == true) text = (chats.sAnnounceOn || this.sAnnounceOn || conn.sAnnounceOn || 'Group telah ditutup!!')
        if (groupUpdate.announce == false) text = (chats.sAnnounceOff || this.sAnnounceOff || conn.sAnnounceOff || 'Group telah dibuka!!')
...
*.delete*`, author, 'Simple Bot OpenAi', 'Matikan Fitur', '.on delete', 'Menu', '.menu', msg, adReply)
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: 'Fitur ini hanya dapat digunakan oleh developer bot!',
        owner: 'Fitur ini hanya dapat digunakan oleh owner bot!',
        mods: 'Fitur ini hanya dapat digunakan oleh moderator bot!',
        premium: 'Fitur ini hanya dapat digunakan oleh pengguna premium!',
        group: 'Fitur ini hanya dapat digunakan di group!',
        private: 'Fitur ini hanya dapat digunakan di chat pribadi!',
        admin: 'Fitur ini hanya dapat digunakan oleh admin!',
        botAdmin: 'Fitur ini dapat digunakan jika bot menjadi admin group!',
        unreg: 'Anda belum terdaftar di database.\nSilahkan ketik *.daftar* untuk menggunakan fitur ini!', 
        restrict: 'Restrict belum dinyalakan dichat ini!',
    }[type]
    if (msg) return conn.send2ButtonDoc(m.chat, msg, author, 'Simple Bot OpenAi', 'Creator', '.creator', 'Menu', '.menu', fakes, adReply)
}
